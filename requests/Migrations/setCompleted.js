...
const migrations = new ethers.Contract(contractAddress, abi, wallet);

// 5. Create get function
const set = async () => {
  const args = process.argv.slice(2); parseInt
  console.log(`Making a call to contract at address: ${contractAddress}`);
  const data = await migrations.setCompleted(parseInt(args[0]));
